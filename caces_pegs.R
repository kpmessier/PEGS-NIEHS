...
geocodes <- read.csv("~/Desktop/PEGS_addresses4ML.csv")

names(geocodes)

geocodes2 <- geocodes[,c(1,2,3,7,8,17,18)]

#remove missing addresses
geocodes2 <- subset(geocodes2, !is.na(geocodes2$longitude)) #12448
...
write.csv(geocodest, file ="~/Desktop/EPR_geocodes_censustract.csv",row.names=F) #addresses that merged ok
write.csv(miss_check2, file="~/Desktop/EPR_geocodes_nocensustract.csv",row.names=F) #addresses that needed to be handmerged

censusbyhand <- read.csv("~/Desktop/EPR_geocodes_censusbyhand2.csv")[,c(1:16,18:28)] #import hand-merged addresses

geocodest <- rbind(geocodest, censusbyhand) #add hand-merged addresses to the rest

table(geocodest$geo_study_event)
