...
roads <- spTransform(roads, CRS("+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs") ) 
  ...
  rd_10kmfun <- function(i){
    lbuff_10km <- gBuffer(l[[i]], width = 10000)
    rd_10km <- tryCatch(raster::crop(roads, extent(lbuff_10km)), error = function(e) e)
    ## Mess with this to make it work in the lapply.
    ifelse(class(rd_10km)[1] == "simpleError", NA, gLength(rd_10km, byid=F))
  ...
  totrd_10km<- lapply(1:length(spdf2), rd_10kmfun)
